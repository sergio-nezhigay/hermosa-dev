...
document.querySelectorAll('[id^="Details-"] summary').forEach((summary) => {
  ...
  summary.addEventListener('click', (event) => {
    ...
    if (detailsElement) {
      event.currentTarget.setAttribute('aria-expanded', !detailsElement.hasAttribute('open'));
    }
  });
...
// QuantityInput class moved to quantity-input.js to avoid conflicts
...
class MenuDrawer extends HTMLElement {
  ...
  closeMenuDrawer(event, elementToFocus = false) {
    ...
    if (this.dataset && this.dataset.breakpoint) {
  ...
  onFocusOut() {
    setTimeout(() => {
      if (this.mainDetailsToggle && this.mainDetailsToggle.hasAttribute('open') && !this.mainDetailsToggle.contains(document.activeElement))
  ...
  closeAnimation(detailsElement) {
    ...
    const handleAnimation = (time) => {
      ...
      if (elapsedTime < 400) {
        ...
        const openDetails = detailsElement.closest('details[open]');
        if (openDetails) {
          const summaryElement = detailsElement.querySelector('summary');
          if (summaryElement) {
            trapFocus(openDetails, summaryElement);
          }
        }
      }
    };
